...
def build_projection_matrix(fov, aspect, zn, zf):
    ...
    proj[0][0] = 1 / (np.tan(fov) * aspect)
    proj[1][1] = 1 / np.tan(fov)
    proj[2][2] = zf / (zf - zn)
    proj[2][3] = 1.0
...
while running:
    ...
    for point in points_3d:
        x, y, z = point
        screen_x = int((x + 2) * 100)  # 簡單平移以可視化
        screen_y = int((2 - y) * 100)
    ...
    text_2d = font.render("Red: Projected 2D Points", True, (255, 0, 0))
